                                ...
                                <table class="table table-lg">
                                    ...
                                    <tbody>
                                        @php
                                            $_devices = []
                                        @endphp
                                        @foreach ($blast->blastdevice as $key => $item)
                                            @php
                                                if($item->device->status == "connected" )
                                                    $_devices[] = $item->device;
                                            @endphp
                                            <tr>
                                                <td>
                                                    {{$key + 1}}
    ...
    <script>
        document.addEventListener('livewire:load', function () {
            ...
            document.querySelector(".btn-mulai-blast").addEventListener("click", (e) => {
                // console.log("masuk")
                _socket.emit("broadcast", { devices: {!! json_encode($_devices)!!}, contacts: {!! json_encode($blastcontact)!!}, message: {!! json_encode(formatPesan($blast)) !!}, id: {{$blast->id}}, delay: "{{getConfig("BROADCAST_DELAY")}}" })
            })
        })
    </script>
</x-app-layout>
