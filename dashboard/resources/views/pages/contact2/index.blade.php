        ...
        <div class="row">
            ...
            <div class="col-12 col-md-6 order-md-2 order-last">
                <div class="d-flex h-100">
                    <form id="form-filter" action="{{ route("kontak2.index") }}" class="row g-2 ms-auto mt-auto mb-3 me-3">
                        <div class="col-auto">
                    ...
                    <form method="POST" enctype="multipart/form-data" action="{{ route('kontak2.import') }}" class="mt-auto ms-2">
                        {{csrf_field()}}
                    ...
                    <a href="{{ route('kontak2.create') }}" class="btn ms-2 btn-primary mt-auto mb-3"><i class="bi bi-plus"></i> Tambah</a>
                    <a href="{{ route('kontak2.delete-all') }}" class="btn ms-2 btn-danger mt-auto mb-3" onclick="return confirm('Are you sure to delete all contact ?');"><i class="bi bi-trash2-fill"></i> Delete All</a>
                </div>
            </div>
        </div>
    </x-slot>
                                    ...
                                    <tbody>
                                        @foreach ($contact as $key => $item)
                                            <tr>
                                                ...
                                                <td>{{ $item->number }}</td>
                                                <td>
                                                    <a href="{{ route('kontak2.edit', ['kontak2' => $item->id]) }}" class="btn btn-sm my-1 btn-warning">
                                                        <i class="bi bi-pen-fill"></i>
                                                    </a>
                                                    <form class="d-inline" action="{{ route('kontak2.destroy', ['kontak2' => $item->id]) }}" method="post">
                                                        @csrf
                                                        @method('DELETE')
